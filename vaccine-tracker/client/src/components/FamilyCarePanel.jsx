...
export default function FamilyCarePanel({ profile, onUpdated }) {
  ...
  const [profileValues, setProfileValues] = useState({ name: profile.name, dob: profile.dob?.slice(0, 10), gender: profile.gender, relationship: profile.relationship });
  ...
  const saveProfile = async (event) => {
    ...
    setMessage('');
    try {
      const { data } = await api.patch(`/profiles/${profile._id}`, profileValues);
      onUpdated(data);
    ...
    <>
      ...
      <div className="card">
        ...
        <form onSubmit={saveRecord} className="pet-form-grid">
          <select value={record.type} onChange={(e) => setRecord({ ...record, type: e.target.value })}>{RECORD_TYPES.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select>
